...
class ContractWindWaveController {
  @Get()
  async getWindWave(@Query('dateFrom') dateFrom?: string, @Query('dateTo') dateTo?: string) {
...
describe('Public API Contract — GET /api/public/wind-wave', () => {
  ...
  it('returns 200 with WindWaveResponse shape per PUBLIC_API.md §3.4', async () => {
    const res = await request(app.getHttpServer()).get('/api/public/wind-wave');
  ...
  it('accepts optional stationId, dateFrom, dateTo', async () => {
    const res = await request(app.getHttpServer()).get(
      '/api/public/wind-wave?stationId=st-001&dateFrom=2026-08-01&dateTo=2026-08-07',
    );
    expect(res.status).toBe(200);
  });
  ...
  it('returns data array matching date range length', async () => {
    const res = await request(app.getHttpServer()).get(
      '/api/public/wind-wave?dateFrom=2026-08-01&dateTo=2026-08-03',
    );
    expect(res.status).toBe(200);
