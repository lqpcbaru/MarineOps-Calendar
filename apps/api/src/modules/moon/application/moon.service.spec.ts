...
class StubMoonProvider implements MoonProviderPort {
  async getMoonPhase(): Promise<MoonDataPoint> {
    return { date: '2026-08-05', phaseName: 'Bulan Penuh', illumination: 100, ageDays: 14, moonrise: '2026-08-05T18:30:00Z', moonset: '2026-08-06T06:00:00Z' };
  }
}
...
describe('MoonService', () => {
  ...
  it('refreshStation returns FAILURE on error', async () => {
    const failing: MoonProviderPort = { async getMoonPhase() { throw new Error('down'); } };
  ...
  it('cache is reused on second call', async () => {
    let calls = 0;
    const counting: MoonProviderPort = { async getMoonPhase() { calls++; return { date: '', phaseName: '', illumination: 0, ageDays: 0, moonrise: null, moonset: null }; } };
