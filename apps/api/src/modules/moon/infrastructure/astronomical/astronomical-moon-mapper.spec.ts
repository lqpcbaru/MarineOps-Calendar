...
describe('AstronomicalMoonMapper', () => {
  it('maps raw data to MoonDataPoint', () => {
    const raw: AstronomicalMoonRawData = {
      julianDate: 2459200, phaseAngle: 180, illumination: 100, ageDays: 14.5,
      phaseName: 'Bulan Penuh', moonrise: '2026-08-06T18:30:00Z', moonset: '2026-08-07T06:00:00Z',
      nextPhase: { name: 'Suku Ketiga', date: '2026-08-13' },
      distanceKm: 384400, angularDiameter: 0.52,
    };
...
describe('MoonEngine', () => {
  it('produces deterministic output', () => {
    const a = computeMoonPhase(new Date('2026-08-06'));
    const b = computeMoonPhase(new Date('2026-08-06'));
    expect(a).toEqual(b);
  });

  it('returns valid phase name', () => {
    const result = computeMoonPhase(new Date('2026-08-06'));
    expect([
      'Bulan Baharu', 'Bulan Sabit Muda', 'Suku Pertama', 'Bulan Hampir Penuh',
      'Bulan Penuh', 'Suku Ketiga', 'Bulan Sabit Tua', 'Tidak Diketahui',
  ...
  it('different dates produce different results', () => {
    const a = computeMoonPhase(new Date('2026-08-01'));
    const b = computeMoonPhase(new Date('2026-08-29'));
