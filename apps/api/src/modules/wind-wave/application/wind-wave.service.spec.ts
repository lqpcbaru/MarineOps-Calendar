...
describe('WindWaveService', () => {
  it('returns WindWaveResponse with data and freshness', async () => {
    const service = new WindWaveService(
      new StubWindWaveProvider(),
      createCache(),
      createStationPort(),
    );
  ...
  it('refreshStation uses Malaysia operational date (localToday) not UTC', async () => {
    ...
    const spy: WindWaveProviderPort = {
      async getWindWave(_stationId, dateFrom) {
    ...
    const service = new WindWaveService(spy, createCache(), createStationPort());
  ...
  it('refreshStation returns FAILURE on error', async () => {
    const failing: WindWaveProviderPort = {
      async getWindWave() {
  ...
  it('cache is reused on second call', async () => {
    let calls = 0;
    const counting: WindWaveProviderPort = {
      async getWindWave() {
        calls++;
        return [
          { date: '', windSpeed: 0, windDirection: '', windGusts: 0, waveHeight: 0, wavePeriod: 0 },
        ];
      },
    };
