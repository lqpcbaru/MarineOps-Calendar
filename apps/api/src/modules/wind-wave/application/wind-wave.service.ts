...
export class WindWaveService {
  constructor(
    ...
    @Inject('CACHE_SERVICE') private readonly cache: CacheService<WindWaveDataPoint[]>,
  ...
  async getWindWave(
    ...
    const result = await this.cache.getOrFetch(
      cacheKey,
      async () => this.provider.getWindWave(stationId, from, to),
      'marineforecast',
      stationId,
    );
    ...
    const freshness: Freshness = {
      ...
      source: result.source === 'cache' ? 'cache' : 'marineforecast',
    };
