...
describe('SunService', () => {
  ...
  it('refreshStation uses Malaysia operational date (localToday) not UTC', async () => {
    ...
    const spy: SunProviderPort = {
      async getSunData(_stationId, date) {
        capturedDate = date;
        return {
          date: '2026-08-05',
          sunrise: '2026-08-05T06:30:00Z',
  ...
  it('cache is reused on second call', async () => {
    let calls = 0;
    const counting: SunProviderPort = {
      async getSunData() {
        calls++;
        return { date: '', sunrise: '', sunset: '', solarNoon: '', daylightDuration: '' };
      },
    };
