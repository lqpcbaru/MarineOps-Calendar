  ...
  async getPublicDashboard(stationId?: string): Promise<DashboardResponse> {
    ...
    return {
      ...
      weather: record?.weather
        ? { conditions: record.weather.conditions, temperature: record.weather.temperature }
        : null,
      wind: record?.windWave
        ? { speed: record.windWave.windSpeed, direction: record.windWave.windDirection }
        : null,
      wave: record?.windWave ? { height: record.windWave.waveHeight } : null,
      tide: record?.tide
        ? { type: record.tide.type, nextHigh: record.tide.nextHigh, nextLow: record.tide.nextLow }
        : null,
      moon: record?.moon
        ? { phaseName: record.moon.phaseName, illumination: record.moon.illumination }
        : null,
