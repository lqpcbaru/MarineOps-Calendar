...
describe('OperationalCalendarService', () => {
  function createService() {
    ...
    const tide: TideService = {
      async getTide() {
        return {
          data: [
            { date: '2026-08-06', time: '2026-08-06T06:30:00Z', height: 2.5, type: 'HIGH' },
            { date: '2026-08-06', time: '2026-08-06T12:45:00Z', height: 0.8, type: 'LOW' },
          ],
          freshness: { status: 'fresh', fetchedAt: '', validUntil: '', source: '' },
        };
      },
  ...
  it('returns exact calendar dates in order for a multi-day range', async () => {
    const svc = createService();
    const result = await svc.getCalendar('st-001', '2026-08-14', '2026-08-20');
  ...
  it('handles single-day range (dateFrom equals dateTo)', async () => {
    const svc = createService();
    const result = await svc.getCalendar('st-001', '2026-08-14', '2026-08-14');
    expect(result.data).toHaveLength(1);
    expect(result.data[0]!.date).toBe('2026-08-14');
  });
  ...
  it('cache is reused on second call', async () => {
    ...
    const svc = new OperationalCalendarService(
      weather,
      tide,
      windWave,
      moon,
