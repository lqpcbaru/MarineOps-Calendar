...
function createCache(): CacheService<WeatherDataPoint[]> {
  return new CacheService(new InMemoryCacheStore(), createCachePolicy({ ttlMs: 30 * 60 * 1000, staleTtlMs: 120 * 60 * 1000 }));
}
...
class StubWeatherProvider implements WeatherProviderPort {
  async getCurrentWeather(): Promise<WeatherDataPoint> {
    return { date: '2026-08-05', temperature: 30, conditions: 'Cerah', visibility: 10, precipitation: 0 };
  }
...
describe('WeatherService', () => {
  it('returns WeatherResponse with data and freshness', async () => {
    const service = new WeatherService(new StubWeatherProvider(), createCache(), createStationPort());
  ...
  it('refreshStation returns FAILURE on provider error', async () => {
    const failing: WeatherProviderPort = {
      async getCurrentWeather() { throw new Error('down'); },
      async getForecast() { throw new Error('down'); },
    };
  ...
  it('cache is reused on second getWeather call', async () => {
    ...
    const countingProvider: WeatherProviderPort = {
      async getCurrentWeather() { providerCalls++; return { date: '', temperature: 0, conditions: '', visibility: 0, precipitation: 0 }; },
      async getForecast() { providerCalls++; return [{ date: '', temperature: 0, conditions: '', visibility: 0, precipitation: 0 }]; },
    };
    const cache = createCache();
  ...
  it('getWeather returns stale when cache is stale and provider fails', async () => {
    const provider: WeatherProviderPort = {
      async getCurrentWeather() { return { date: '', temperature: 0, conditions: '', visibility: 0, precipitation: 0 }; },
      async getForecast() { throw new Error('down'); },
    };
    const cache = createCache();
    ...
    await cache.set(key, [{ date: '2026-08-05', temperature: 25, conditions: 'StaleData', visibility: 5, precipitation: 0 }], 'metmalaysia', 'st-001');
