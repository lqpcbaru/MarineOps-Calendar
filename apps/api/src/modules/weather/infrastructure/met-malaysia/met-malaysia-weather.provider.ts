...
export class MetMalaysiaWeatherProvider implements WeatherProviderPort {
  ...
  async getCurrentWeather(stationId: string): Promise<WeatherDataPoint> {
    ...
    try {
      ...
      const data = await this.retry.execute(async () => {
        const response = await this.httpClient.get<MetRawForecastResponse>(
          '/v2/forecast',
          { area, type: 'current' },
        );
      ...
      this.logger.requestFailed('getCurrentWeather', error instanceof Error ? error.message : 'unknown', 1);
      throw this.classifyError(error);
    }
  }

  async getForecast(stationId: string, dateFrom: string, dateTo: string): Promise<WeatherDataPoint[]> {
    const start = Date.now();
    ...
    try {
      ...
      this.metrics.recordFailure(error instanceof Error ? error.message : 'unknown');
      this.logger.requestFailed('getForecast', error instanceof Error ? error.message : 'unknown', 1);
      throw this.classifyError(error);
    }
  }
  ...
  private async resolveArea(stationId: string, dataType: string): Promise<string> {
    ...
    if (!mapping || !mapping.isActive) {
      throw new ProviderInvalidResponseError('MetMalaysia', `tiada pemetaan untuk stesen ${stationId}`);
    }
    const area = (mapping.config as Record<string, unknown> | null)?.marineArea as string
  ...
  private emptyPoint(): WeatherDataPoint {
    return { date: new Date().toISOString().slice(0, 10), temperature: 0, conditions: 'UNKNOWN', visibility: 0, precipitation: 0 };
  }
}
